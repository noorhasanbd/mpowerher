...
export default async function AdminDashboardPage() {
  ...
  const stats = [
    { label: "Total Users", value: "1,420", icon: Users, color: "text-purple-600", bg: "bg-purple-50" },
    { label: "Active Courses", value: "18", icon: BookOpen, color: "text-[#C01C5C]", bg: "bg-pink-50" },
    { label: "System Health", value: "99.9%", icon: Server, color: "text-emerald-600", bg: "bg-emerald-50" },
    { label: "Active Issues", value: "1", icon: AlertTriangle, color: "text-amber-600", bg: "bg-amber-50" },
  ];

  const recentUsers = [
    { id: "u1", name: "Elena Rostova", email: "elena@example.com", role: "Student", date: "10 mins ago" },
    { id: "u2", name: "Dr. Marcus Vance", email: "m.vance@example.com", role: "Educator", date: "1 hour ago" },
    { id: "u3", name: "Clara Oswald", email: "clara@example.com", role: "Student", date: "3 hours ago" },
  ];

  const systemLogs = [
    { id: "l1", type: "Security", message: "Failed login attempt detected from IP 192.168.1.4", time: "12 mins ago", severity: "warning" },
    { id: "l2", type: "System", message: "Automated database backup completed successfully", time: "2 hours ago", severity: "info" },
    { id: "l3", type: "Auth", message: "New Educator role granted to m.vance@example.com", time: "1 hour ago", severity: "info" },
  ];
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-purple-800 to-purple-600 p-8 text-white shadow-lg shadow-purple-200/50">
        <div className="relative z-10 max-w-2xl space-y-3">
          ...
          <p className="text-sm font-medium text-purple-100 sm:text-base">
            All services are running normally. 24 new platform registrations were logged in the past 24 hours.
          </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="font-heading text-xl font-bold text-slate-800">Recent Registrations</h2>
            <Link href="/admin/users" className="text-sm font-bold text-[#C01C5C] hover:underline">
              View User Roster
            </Link>
          </div>
                ...
                <div key={user.id} className="p-4 sm:px-6 flex items-center justify-between hover:bg-slate-50 transition-colors">
                  ...
                  <div className="flex items-center gap-4">
                    <span className={`text-xs font-bold px-2.5 py-1 rounded-md ${
                      user.role === 'Educator' ? 'bg-amber-100 text-amber-800' : 'bg-pink-100 text-[#C01C5C]'
                    }`}>
                      {user.role}
                    </span>
                    <span className="text-xs text-slate-400 hidden sm:inline-block">{user.date}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="font-heading text-xl font-bold text-slate-800">System Logs</h2>
            <Link href="/admin/system-logs" className="text-sm font-bold text-[#C01C5C] hover:underline">
              Logs Console
            </Link>
          </div>
          ...
          <div className="space-y-3">
            {systemLogs.map((log) => (
              <div key={log.id} className="rounded-2xl p-4 bg-white border border-slate-100 shadow-sm space-y-2">
                <div className="flex items-center justify-between">
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider ${
                    log.severity === 'warning' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-600'
                  }`}>
                    {log.type}
                  </span>
